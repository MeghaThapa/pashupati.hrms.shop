...
class FinalTripalReportController extends Controller
{
    protected $neDate;
    ...
    public function finaltripalReport()
    {
        $godams = Godam::all();
        return view('admin.report.finaltripalreport.index', compact('godams'));
    }
    ...
    public function generateDoubleTripalView(Request $request)
    {
        ...
        foreach ($nepaliDates as $nepaliDate) {
            ...
            $fabricGodams = DoubleSideLaminatedFabric::where('bill_number','!=','opening')->where('bill_date', $nepaliDate);
            ...
            if (!$fabricGodams->isEmpty()) {
                $fabricView = view('admin.report.finaltripalreport.finaltripal_transfer_datewise', compact('fabricGodams', 'nepaliDate'))->render();
        ...
        $summaryFabrics = DoubleSideLaminatedFabric::where('bill_number','!=','opening')->select(
            'name',
        ...
        $summaryView = view('admin.report.finaltripalreport.summary', compact('summaryFabrics', 'nepaliStartDate', 'nepaliEndDate'))->render();
