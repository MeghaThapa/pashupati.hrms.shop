    ...
    public function index()
    {
        $tableDatas = DB::table('tape_entry_openings')
            ->select(
                ...
                'godam.id',
                'godam.name',
                DB::raw('(SELECT SUM(total_netweight) FROM fabric_details WHERE fabric_details.godam_id = tape_entry_openings.godam_id) AS rolldown_total'),
                ...
                DB::raw('(SELECT SUM(tape_qty_in_kg) FROM tape_entry_items WHERE tape_entry_items.toGodam_id = tape_entry_openings.godam_id) AS production_total')
            )
