...
class TapeEntryController extends Controller
{
    ...
    public function edit(TapeEntry $tapeEntry)
    {
        $departments = [];
        ...
        $department = Godam::whereIn("id", $departments)->get();
        $shift = AutoLoadItemStock::with('shift')->get();
        $tapeentries = TapeEntry::where('id', $tapeEntry->id)->get();
        $wastage = Wastages::all();
        $tapeDanaItems = TapeDanaItem::with('godam', 'danaName', 'danaGroup')->where('tape_entry_id', $tapeEntry->id)->get();
        return view('admin.TapeEntry.edit', compact('tapeEntry', 'department', 'shift', 'tapeentries', 'wastage', 'tapeDanaItems'));
    }
    ...
    public function tapeentrystockstore(Request $request)
    {
        try {
            ...
            $autoloadItems = AutoloadItems::where('from_godam_id', $department)
                ->where('plant_type_id', $planttype)
                ->where('plant_name_id', $plantname)
                ->where('shift_id', $shift)
                ->where('transfer_status', 'Autoload')
                ->get();

            foreach ($autoloadItems as $item) {
                $tapeDanaItem = new TapeDanaItem();
                $tapeDanaItem->tape_entry_id = $tape_entry_id;
                $tapeDanaItem->from_godam_id = $item->from_godam_id;
                $tapeDanaItem->plant_type_id = $item->plant_type_id;
                $tapeDanaItem->plant_name_id = $item->plant_name_id;
                $tapeDanaItem->shift_id = $item->shift_id;
                $tapeDanaItem->dana_group_id = $item->dana_group_id;
                $tapeDanaItem->dana_name_id = $item->dana_name_id;
                $tapeDanaItem->quantity = $item->quantity;
                $tapeDanaItem->autoload_id = $item->id;
                $tapeDanaItem->save();
                $item->transfer_status = "Tape";
                $item->save();
            }
    ...
    {
        $tapeEntry = TapeEntry::findOrFail($request->tape_entry_id);
        $tapeDanaItem = TapeDanaItem::findOrFail($request->restore_id);
        try {
            DB::beginTransaction();

            $autoLoadStockExists = AutoLoadItemStock::where('from_godam_id', $tapeDanaItem->from_godam_id)
                ->where('plant_type_id', $tapeDanaItem->plant_type_id)
                ->where('plant_name_id', $tapeDanaItem->plant_name_id)
                ->where('shift_id', $tapeDanaItem->shift_id)
                ->where('dana_group_id', $tapeDanaItem->dana_group_id)
                ->where('dana_name_id', $tapeDanaItem->dana_name_id)
                ->first();

            if ($autoLoadStockExists) {
                $autoLoadStockExists->quantity = $autoLoadStockExists + $tapeDanaItem->quantity;
                $autoLoadStockExists->save();
            } else {
                AutoLoadItemStock::create([
                    'from_godam_id' => $tapeDanaItem->from_godam_id,
                    'plant_type_id' => $tapeDanaItem->plant_type_id,
                    'plant_name_id' => $tapeDanaItem->plant_name_id,
                    'shift_id'      => $tapeDanaItem->shift_id,
                    'dana_group_id' => $tapeDanaItem->dana_group_id,
                    'dana_name_id'  => $tapeDanaItem->dana_name_id,
                    'quantity'      => $tapeDanaItem->quantity,
                ]);
            }

            $autoLoadItem = AutoloadItems::where('id', $tapeDanaItem->autoload_id)->first();
            ...
            $tapeDanaItems = TapeDanaItem::with('godam', 'danaName', 'danaGroup')->where('tape_entry_id', $tapeEntry->id)->get();
            $view = view('admin.TapeEntry.ssr.danaview', compact('tapeDanaItems'))->render();

            return response(['status'=>true,'message'=>'Restored Successfully','view'=>$view]);
        } catch (\Exception $e) {
            DB::rollBack();
