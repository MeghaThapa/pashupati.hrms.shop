...
class SaleFinalTripalController extends Controller
{
        ...
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->status == "sent") {
                    ...
                    return '<a href="' . route('salefinaltripals.viewTripalBill', $row->id) . '" class="btn btn-primary" ><i class="fas fa-print"></i></a>';
                }
    ...
    public function viewTripalBill(SaleFinalTripal $saleFinalTripal)
    {
        $fabrics = FinalTripalStock::get()->unique('name')->values()->all();
        $salefinaltripals = SaleFinalTripalList::where('salefinal_id', $saleFinalTripal->id)->orderBy('name')->get();

        $total_gross = SaleFinalTripalList::where('salefinal_id', $saleFinalTripal->id)->sum('gross');
        $total_net = SaleFinalTripalList::where('salefinal_id', $saleFinalTripal->id)->sum('net');
        $total_meter = SaleFinalTripalList::where('salefinal_id', $saleFinalTripal->id)->sum('meter');

        $totaltripals = SaleFinalTripalList::where('salefinal_id', $saleFinalTripal->id)->select('name', DB::raw('SUM(gross) as total_gross'), DB::raw('SUM(net) as total_net'), DB::raw('SUM(meter) as total_meter'), DB::raw('COUNT(name) as total_count'))
            ->groupBy('name')
            ->get();
        return view('admin.sale.salefinaltripal.billtripallist', compact('saleFinalTripal', 'fabrics', 'salefinaltripals', 'totaltripals', 'total_gross', 'total_net', 'total_meter'));
    }
    ...
    {
        return view('admin.sale.salefinaltripal.report');
    }

    public function reportGenerate(Request $request)
    {
        $saleFinalTripals = SaleFinalTripal::with('getSaleList')
