...
class NonwovenSaleController extends Controller
{
    ...
    {
        return view('admin.sale.nonwovensale.report');
    }

    public function generateReport(Request $request)
    {
        $nonWovenSales = NonwovenSale::with('nonwovenSaleEntry')
        ...
        $dataView = view('admin.sale.nonwovensale.ssr.reportview', compact('nonWovenSales'))->render();

        $summaryReport = DB::table('nonwoven_sale_entries')
            ->leftJoin('nonwoven_sales', 'nonwoven_sale_entries.bill_id', 'nonwoven_sales.id')
            ->select(
                'nonwoven_sale_entries.fabric_name',
                'nonwoven_sale_entries.fabric_gsm',
                'nonwoven_sale_entries.fabric_color',
                DB::raw('COUNT(*) as fabric_count'), // Count all rows in the group
                DB::raw('SUM(nonwoven_sale_entries.length) as total_length'),
                DB::raw('SUM(nonwoven_sale_entries.gross_weight) as total_gross_wt'),
                DB::raw('SUM(nonwoven_sale_entries.net_weight) as total_net_wt')
            )
            ->where('nonwoven_sales.bill_date', '>=', $request->start_date)
            ->where('nonwoven_sales.bill_date', '<=', $request->end_date)
            ->where('nonwoven_sales.bill_for', '=', $request->bill_for)
            ->groupBy(
                'nonwoven_sale_entries.fabric_name',
                'nonwoven_sale_entries.fabric_gsm',
                'nonwoven_sale_entries.fabric_color'
        ...
        $summaryView = view('admin.sale.nonwovensale.ssr.report_summary_view', compact('summaryReport'))->render();
