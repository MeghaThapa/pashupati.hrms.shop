...
@section('content')
    ...
    <div class="content">
        <div class="container-fluid">
            <form id="tape_entry_form" action="{{ route('tape.entry.stock.store') }}" method="post">
                ...
                @endforeach
                <input id="tape_entry_id" type="text" name='tape_entry_id' value="{{ $tapeEntry->id }}" style="display: none;">
                <hr>
                <div class='row mt-2'>
                    <div class="col-md-12">
    ...
    @push('scripts')
        <script>
            ...
            $(document).on('click','.item_recover',function(){
                let restore_id = $(this).data('id');
                $('#restore_id').val(restore_id);
                $.ajax({
                    url: "{{ route('tape.entry.restore') }}",
                    method: "post",
                    data: {
                        "_token": $("meta[name='csrf-token']").attr("content"),
                        "tape_entry_id": $('#tape_entry_id').val(),
                        "restore_id": restore_id,
                    },
                    success: function(response) {
                        if(response.status==true){
                            $('#tableContent').html(response.view);
                        }
