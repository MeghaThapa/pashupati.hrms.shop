...
@section('content')
    ...
    <div class="content-header mb-4">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">{{ __('Processing Department') }}</h1>
            </div>
            <div class="col-sm-6">
                                    ...
                                    <td class="text-right">
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                @if($unit->isActive())
                                                    <a href="{{ route('department.status', $unit->slug)  }}" class="dropdown-item"><i class="fas fa-window-close"></i> {{ __('Inactive') }}</a>
                                                @else
                                                    <a href="{{ route('department.status', $unit->slug)  }}" class="dropdown-item"><i class="fas fa-check-square"></i> {{ __('Active') }}</a>
                                                @endif
                                                <a href="{{ route('department.edit', $unit->slug) }}" class="dropdown-item"><i class="fas fa-edit"></i> {{ __('Edit') }}</a>
                                                <a href="{{ route('department.delete', $unit->slug)  }}" class="dropdown-item delete-btn" data-msg="{{ __('Are you sure you want to delete this unit?') }}"><i class="fas fa-trash"></i> {{ __('Delete') }}</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
