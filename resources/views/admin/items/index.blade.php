        ...
        <div class="container-fluid">
            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-7 col-6">
                    <div class="card-tools text-md-right">
                        {{-- <a class="btn btn-secondary" href="{{ route('items.pdf') }}">
                            <i class="fas fa-download"></i> @lang('Export')
                        </a> --}}
                        <a href="{{ route('items.create') }}" class="btn btn-primary">
                            {{ __('Add Item') }} <i class="fas fa-plus-circle"></i>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="p-0 table-responsive table-custom my-3">
                <table class="table">
                    ...
                    @if ($items->total() > 0)
                        @foreach ($items as $key => $subCat)
                            <tr>
                                <td>{{ ++$key }}</td>
                                <td>{{ $subCat->item }} </td>
                                <td>{{ $subCat->pnumber }} </td>
                                ...
                                {{-- <td class="text-right">
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                             @if ($subCat->isActive())
                                                <a href="{{ route('items.status', $subCat->slug) }}"
                                            ...
                                            @else
                                                <a href="{{ route('items.status', $subCat->slug) }}"
                                                    ...
                                                    {{ __('Active') }}</a>
                                            @endif
                                            <a href="{{ route('items.edit', $subCat->slug) }}"
                                                class="dropdown-item"><i class="fas fa-edit"></i>
                                                {{ __('Edit') }}</a>
                                            <a href="{{ route('items.delete', $subCat->slug) }}"
                        ...
                        <tr>
                            <td colspan="10">
                                <div class="data_empty">
                                    ...
                                    <p>{{ __('Sorry, no items found in the database. Create your very first sub category.') }}
                                    </p>
                                    <a href="{{ route('items.create') }}" class="btn btn-primary">
                                        {{ __('Add Items') }} <i class="fas fa-plus-circle"></i>
                                    </a>
                                </div>
                            </td>
                        </tr>
