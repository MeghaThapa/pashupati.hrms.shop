        ...
        <form id="createRawMaterial">
            ...
            <div class="row">
                <div class="col-md-2 form-group">
                    ...
                    <input type="text" class="form-control" id="billnumber" value="{{ $bill_no }}"
                        name="bill_number" required /> {{-- value="FSR-{{ getNepalidate(date('Y-m-d')).'-'.rand(0,9999)}}" --}}
                </div>
                ...
                <div class="col-md-2 form-group">
                    ...
                    <input type="date" value="{{ date('Y-m-d') }}" step="any" min="0"
                        class="form-control calculator" id="billDate" data-number="1" name="bill_date"
                ...
                <div class="card-body">
                    <div class="row p-2">
                        <div class="col-md-6">
                            ...
                            <select class="advance-select-box form-control" id="danaNameId" name="danaNameId" disabled
                                required>
                                <option value="" selected disabled>{{ __('--Select Plant Name--') }}</option>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id='sendtolaminationform' action='{{ route('fabricSendReceive.store.laminated') }}'
                        method="post">
                        @csrf
                        <div class="card">
                            <div class="card-body">
    ...
    <script>
        ...
        function filltable(data) {
            ...
            data.response.forEach(d => {
                ...
                tr.append(
                    `<td><div class="btn-group"><a id="sendforlamination" data-group='${d.gram}' data-standard='${result}' data-title='${d.fabric.name}' href="${d.id}" data-id="${d.id}" class="btn btn-info">Send</a><a id="deletesendforlamination" class="btn btn-danger" data-id="${d.id}">delete</a></div></td>`
                );
            });
        }
        ...
        function deletefromunlamintedtable(data) {
            $.ajax({
                url: "{{ route('fabricSendReceive.send.unlaminated.delete', ['id' => ':id']) }}".replace(':id',
                    data),
                method: 'get',
                beforeSend: function() {
        ...
        $("#autoloader_godam").change(function(e) {
            ...
            $.ajax({
                url: "{{ route('get.autoloader.godam.id.into.fsr', ['godamId' => ':godamId']) }}".replace(
                    ":godamId", godamId),
                method: 'get',
                success: function(response) {
