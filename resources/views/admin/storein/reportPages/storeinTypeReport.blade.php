...
@section('content')
    ...
    <div class="content-header mb-4">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">{{ __('Report According to StoreinType') }}</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">{{ __('StoreinType Report') }}</li>
                </ol>
            </div>
        </div>
    </div>
        ...
        <div class="container-fluid">
            <div class="card">
                ...
                <div class="row p-2">
                    ...
                    <div class="col-sm-3">
                        <label for="godamID">Storein Type</label>
                        <select class="form-control" id="storeinTypeId" name="storein_type_id">
                            <option value="" selected disabled>{{ __('Select storein Type') }}</option>
                            @foreach ($storeinTypes as $storeinType)
                                <option value="{{ $storeinType->id }}">{{ $storeinType->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
        $(function() {
            ...
            $("#generateReport").click(function(e) {
                e.preventDefault()
                $.ajax({
                    url: "{{ route('storein.itemStoreinTypeReport.view') }}",
                    method: "post",
                    data: {
                        "_token": $("meta[name='csrf-token']").attr("content"),
                        "start_date": $('#start_date').val(),
                        "end_date": $('#end_date').val(),
                        "storein_type_id": $('#storeinTypeId').val(),
